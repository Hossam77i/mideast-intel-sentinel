...
async function fetchHistoricalArchives() {
  ...
  try {
    ...
    const res = await fetch("/api/collect-historical", {
      method: "POST",
...
async function loadCausalChains() {
  ...
  try {
    ...
    const res = await fetch(`/api/causal-chains?${params.toString()}`);
...
async function loadSettings() {
  try {
    const res = await fetch(`/api/settings?username=${encodeURIComponent(currentUser)}`);
